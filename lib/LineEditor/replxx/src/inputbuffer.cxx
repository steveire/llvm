...
void InputBuffer::highlight( int highlightIdx1, int highlightIdx2, bool error_ ) {
	...
	if ( highlightIdx1 != -1 && highlightIdx2 != -1 ) {
		colors[highlightIdx1] = error_ ? Replxx::Color::ERROR : Replxx::Color::YELLOW;
		colors[highlightIdx2] = error_ ? Replxx::Color::ERROR : Replxx::Color::YELLOW;
	}
	_display.clear();
	_displayPos = 0;
...
int InputBuffer::handle_hints( PromptBase& pi, HINT_ACTION hintAction_ ) {
	_hint = Utf32String();
	int len( 0 );
    bool inHintPosition = false;
      if (_pos == _len || (_buf32[_pos] == ')' || _buf32[_pos] == ' ')) {
          inHintPosition = true;
      }

	if ( !_replxx.no_color() && ( hintAction_ != HINT_ACTION::SKIP ) && _replxx.has_hinter() && inHintPosition) {
		if ( hintAction_ == HINT_ACTION::REGENERATE ) {
			_hintSelection = -1;
		}
		Replxx::Color c( Replxx::Color::BRIGHTCYAN );
		...
		if ( hintCount == 1 ) { //  && ( _pos == _len )
			setColor( c );
			_hint = hints.front();
			len = _hint.length();
			for ( int i( 0 ); i < len; ++ i ) {
                _display.insert(_display.begin() + i + _displayPos, _hint[i] );
            }
			setColor( Replxx::Color::DEFAULT );
			...
			setColor( c );
			if ( _hintSelection != -1 ) {
				std::string hint = Utf8String(hints[_hintSelection]).get();
				auto needle = hint.find(" - ");
				if (needle != std::string::npos)
				{
					_hint = Utf32String(hint.substr(0, needle).c_str());
				} else 
				{
...
void InputBuffer::getBoundaryPoints(int& idx1, int& idx2)
{
	idx1 = -1;
	idx2 = -1;

	unsigned char openingPart = 0;
	unsigned char closingPart = 0;

	int highlightStartPos = _pos;
	if (_pos == _len && strchr("}])", _buf32[_pos - 1]) != nullptr)
	{
		--highlightStartPos;
	}
	...
	for (int i = highlightStartPos - (strchr("{[(", _buf32[highlightStartPos]) == nullptr ? 1 : 0); i >= 0; --i) {
		...
		if (strchr(")]}", _buf32[i])) {
			...
			if (closersSeen == 0) {
				idx1 = i;
				openingPart = _buf32[i];
				switch(_buf32[i])
				{
					case '{': closingPart = '}'; break;
					case '(': closingPart = ')'; break;
					case '[': closingPart = ']'; break;
				}
				break;
	...
	int pendingOpeners = 0;

	for (int i = highlightStartPos + (strchr("[{(", _buf32[highlightStartPos]) != nullptr ? 1 : 0); i  < _len; ++i) {
		
		if (_buf32[i] == openingPart) {
			++pendingOpeners;
		} else if (_buf32[i] == closingPart) {
			if (pendingOpeners == 0)
			{
				idx2 = i;
				break;
...
void InputBuffer::refreshLine(PromptBase& pi, HINT_ACTION hintAction_) {
	...
	int highlightIdx1 = -1;
	int highlightIdx2 = -1;
	getBoundaryPoints(highlightIdx1, highlightIdx2);
	bool indicateError = false;

	highlight( highlightIdx1, highlightIdx2, indicateError );
...
int InputBuffer::completeLine(PromptBase& pi) {
	...
	if ( completionsCount == 1) {
		std::string hint = Utf8String(completions[selectedCompletion]).get();
		auto needle = hint.find(" - ");
		if (needle != std::string::npos)
		{
			hasDash = true;
			longestCommonPrefix = needle + 2;
		} else {
	...
	if ( ( longestCommonPrefix > itemLength ) || ( completionsCount == 1 ) ) {
		...
		auto& textToInsert = completions[selectedCompletion];

		std::string trailingBits;

		auto textData = textToInsert.get();
		auto textDataEnd = textToInsert.get() + longestCommonPrefix;

		auto it = std::find_if_not(textData, textDataEnd, [](char32_t c) { return c != 2; });
		if (it != textDataEnd)
		{
			Utf32String remaining(it, std::distance(it, textDataEnd));
			trailingBits = Utf8String(remaining).get();
			textToInsert = Utf32String(textData, std::distance(textData, it));
		}

		displayLength -= trailingBits.size();
		longestCommonPrefix -= trailingBits.size();

		if ( hasDash ) {
			textToInsert[longestCommonPrefix - 2] = '(';
			textToInsert[longestCommonPrefix - 1] = ')';
			trailingBits = std::string(1, 2);
		}

		// std::string hint = Utf8String(textToInsert).get();
		// auto needle = hint.find(" - ");
		// if (needle != std::string::npos)
		// {
		// 	hint[needle] = '(';
		// 	hint[needle + 1] = ')';
		// 	hint[needle + 2] = 0;
		// 	std::cout << "NEEDLE" << hint; 
		// 	textToInsert = Utf32String(hint.c_str());
		// 	trailingBits = std::string(1, 2);
		// }

		Utf32String displayText(displayLength + 1);
		memcpy(displayText.get(), _buf32.get(), sizeof(char32_t) * startIndex);
		memcpy(&displayText[startIndex], &textToInsert[0],
...
int InputBuffer::getInputLine(PromptBase& pi) {
	...
	while (true) {
		...
		switch (c) {
			case ctrlChar('X'): {	// ctrl-X, move cursor to start of line
				_killRing.lastAction = KillRing::actionOther;
				int idx1 = -1;
				int idx2 = -1;
				getBoundaryPoints(idx1, idx2);
				if (_pos == idx1)
					_pos = idx2;
				else
					_pos = idx1;	
